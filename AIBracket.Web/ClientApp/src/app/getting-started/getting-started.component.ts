...
const EVENT_DATA: PacmanEvent[] = [
  {
    name: "Board Reset", enum: 0, example: "0 28 31 0 0 1 1 5... (28 * 31 total elements)", description: "Packet is recieved when the game is started or the board is reset. The first two numbers are width and height of the board " 
  ...
  { name: "Fruit Consumed", enum: 4, example: "4 15 4 100", description: "Packet is recieved whenever a fruit is consumed and is followed by the X and Y position of said fruit and a score increment." },
  { name: "Power Up Consumed", enum: 5, example: "5 2 18 50", description: "Packet is recieved whenever a power up is consumed and is followed by the X and Y position of said power up and a score increment." },
  ...
  { name: "Ghost Spawn/Despawn", enum: 8, example: "8 1 10", description: "Packet is recieved whenever a ghost dies and is followed by a 0-3 specifying which ghost and a score increment." },
  { name: "Score", enum: 9, example: "1 350", description: "Packet is recieved at the start of the game and is followed by the score. This is intended for specators joining mid-way through a game." }
];
